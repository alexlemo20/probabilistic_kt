    ...
    for epoch in range(epochs):
        ...
        for (inputs, targets) in tqdm(transfer_loader):
            ...
            if supervised_weight > 0:
                ...
                loss = cosine_similarity_loss(outputs_net, output_target, targets, typical_mask, atypical_mask, atypical_proportion) #par_step
...
def calculate_mask(cosine_similarity, labels, typical_mask, atypical_mask, atypical_proportion):
    ...
    #  x, y = coordinates.split(' ')
    #  typical_similarities[int(x)][int(y)] = ((abs(intra_sorted[coordinates] - mean_intra) / max_dis_intra) + 0.5) * atypical_mask
    #  typical_similarities[int(y)][int(x)] = ((abs(intra_sorted[coordinates] - mean_intra) / max_dis_intra) + 0.5) * atypical_mask

    #for coordinates in atypical_inter:
    #  x, y = coordinates.split(' ')
    #  typical_similarities[int(x)][int(y)] = ((abs(inter_sorted[coordinates] - mean_inter) / max_dis_inter)  + 0.5) * atypical_mask
    #  typical_similarities[int(y)][int(x)] = ((abs(inter_sorted[coordinates] - mean_inter) / max_dis_inter)  + 0.5) * atypical_mask

    #for coordinates in typical_intra:
    #  x, y = coordinates.split(' ')
    #  typical_similarities[int(x)][int(y)] = ((abs(intra_sorted[coordinates] - mean_intra) / max_dis_intra) + 0.5) * typical_mask
    #  typical_similarities[int(y)][int(x)] = ((abs(intra_sorted[coordinates] - mean_intra) / max_dis_intra) + 0.5) * typical_mask
  
    #for coordinates in atypical_inter:
    #  x, y = coordinates.split(' ')
    #  typical_similarities[int(x)][int(y)] = ((abs(inter_sorted[coordinates] - mean_inter) / max_dis_inter)  + 0.5) * typical_mask
    #  typical_similarities[int(y)][int(x)] = ((abs(inter_sorted[coordinates] - mean_inter) / max_dis_inter)  + 0.5) * typical_mask



    # 3rd method
    #intra
    #min_intra = intra_sorted.values()[-1]
    #max_dis_intra = intra_sorted.values()[0] - intra_sorted.values()[-1]
    #inter
    #max_inter = inter_sorted.values()[-1]
    #max_dis_inter = inter_sorted.values()[-1] - inter_sorted.values()[0]
    #typical_similarities = torch.cuda.FloatTensor(cosine_similarity.shape[0], cosine_similarity.shape[1]).fill_(1)
